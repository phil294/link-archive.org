...
export default ->
	...
	axios.interceptors.response.use (response => response), error =>
		...
		#if error.response && error.response.status == 401 # fixme reactivate: put this into some kind of global exception handler. it does not belong in this network interceptor as cought exceptions should not log out the user by themselves
		#	store.dispatch 'session/logout'	
		#	router.push '/login'
		#else if
		if error.response == undefined or error.code == 'ECONNABORTED'
