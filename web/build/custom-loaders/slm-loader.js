(function() {
  ...
  module.exports = function(slmdoc) {
    ...
    return slm.compile(slmdoc.replace(/# .*/g, '').replace(/(?<=\s[a-z.@%:-]+=)([^\s"]+)(?=\s|$)/g, '"$1"').replace(/(?<=\s)%(?=[a-z.-]+="[^"]+"\s|$)/g, '@').replace(/(?<=\s)-(?=[a-z.-]+="[^"]+"\s|$)/g, ':').replace(/(?<=\s)model(?=="[^"]+"\s|$)/g, 'v-model').replace(/(?<=\s)if(?=="[^"]+"\s|$)/g, 'v-if').replace(/(?<=\s)(v-else|required|disabled|draggable|selected|)(?=\s|$)/g, '$1=""'))(); // [WS]%src= // Add quotes // [WS] // Replace with @ // [WS] // Replace with v-model // [WS] // Add =""
